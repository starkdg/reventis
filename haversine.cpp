...
double haversine_distance(const Position &a, const Position &b){
	double longitude_delta = (b.lon - a.lon)*m_pi/180.0;
	double latitude_delta = (b.lat - a.lat)*m_pi/180.0;
	...
	double latitude_cos = cos(a.lat*m_pi/180.0)*cos(b.lat*m_pi/180.0);
...
void get_xyradius(const Position &center, const double radius, double &delta_x, double &delta_y){
	double tmp =  sin(radius/(2*m_earth_radius))/cos(center.lat*m_pi/180.0);
